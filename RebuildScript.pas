...
uses Windows, SysUtils, Classes, Main, PathFunc, MD5, SHA1, Struct;
...
procedure TScriptBuilder.PrintSetupHeader(const sh: TSetupHeader);
  ...
  function GetImageFileList(ImgList: TStrings; IsSmalImage: boolean) : String;
  var
    i: Integer;
    NameList: TStrings;
  begin
    ...
    for i := 0 to ImgList.Count - 1 do
      NameList.Add(GetWizardImageFileName(i, IsSmalImage));
...
begin
  ...
  if (Int64(sh.ExtraDiskSpaceRequired) > 0) then
    IntConst('ExtraDiskSpaceRequired', Int64(sh.ExtraDiskSpaceRequired));
  ...
  StrConst('WizardSmallImageFile', GetImageFileList(WizardImages, true));
